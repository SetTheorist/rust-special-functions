...
pub fn erf_series<V:Value+Exp>(x:V) -> V {
  ...
  sf_exp(-x.sqr()) * tqp * sum_series(terms, V::mu_epsilon)
}
...
pub fn erfc_contfrac<V:Value+Exp>(x:V) -> V {
  ...
  sf_exp(-x2) / sqpi * x / contfrac_modlentz(x2, terms, V::mu_epsilon)
}
...
pub fn erfc_contfrac2<V:Value+Exp>(x:V) -> V {
  ...
  let terms = (1..).map(|n|(
    ι(-(2*n-1)*(2*n)):V,
    x2+(4*n+1)));
  sf_exp(-x.sqr()) / sqpi * (x*2) / contfrac_modlentz(x2+1, terms, V::mu_epsilon)
}
