\documentclass[10pt,dvipdfm,letterpaper,twoside]{article}
...
$2^x$, $10^x$, $e^z-1-z$, $\frac{e^z-1-z}{z^2}$,
$e_N(x)=\sum_{n=0}^N\frac{x^n}{n!}$, $e^z - e_N(z)$, $\frac{e^z - e_N(z)}{z^{N+1}}$
...
\begin{implementation}
\begin{itemize}
\item continued fraction $\tan z = \frac{z}{1-} \frac{z}{3-} \frac{z}{5-} \cdots$
...
\[ \gam(z,\xi) = \int_0^\xi t^{z-1}e^{-t}\,dt   \DEF{$\gam(z,\xi)$}\]
\[ \gam^*(z,\xi) = z^{\dots}\gam(z,\xi) \]
\[ \O{P}(z,\xi) = \frac{\gam(z,\xi)}{\Gam(z)}   \DEF{$\O{P}(z,\xi)$}\]
\[ \O{Q}(z,\xi) = 1 - \O{P}(z,\xi) = \frac{\Gam(z,\xi)}{\Gam(z)}    \DEF{$\O{Q}(z,\xi)$}\]
\[ \Gam(z,\xi) = \Gam(z) - \gam(z,\xi) = \int_\xi^\infty t^{z-1}e^{-t}\,dt \DEF{$\Gam(z,\xi)$}\]
For a fixed $\xi$, $\gamma(z,\xi)$ is a meromorphic function of $z$ with simple poles at $z=0,-1,-2,\dots$,
while $\Gam(z,\xi)$ is an entire function of~$z$.
...
\begin{itemize}
\item[\bf(A)] For $a\neq-1,-2,\dots$,
\[ \Gam(a,z) = \Gam(a) - \sum_{n=0}^\infty\frac{\Gam(a)e^{-z}z^{n+a}}{\Gam(a+n+1)} = \Gam(a) - \sum_{n=0}^\infty\frac{(-)^nz^{n+a}}{(a+n)n!} \]
\item[\bf(B)] As $z\to\infty$,
\[ \Gam(a,z)\sim z^{a-1}e^{-z}\sum_{n=0}^\infty\frac{\Gam(a)}{\Gam(a-n)z^n} \]
\item[\bf(C)] Continued fraction expansion: (best when $a>>z$)
\[ \Gam(a,z) = \frac{e^{-z}z^a}{z+} \frac{1-a}{1+} \frac{1}{z+} \frac{2-a}{1+} \frac{2}{z+} \cdots \]
\item[\bf(C')] Equivalent fraction (converges for $\Re z>0$), where $v=z^{-1}$
\[ e^{z}z^{1-a}\Gam(a,z) = \frac{1}{1+} \frac{(1-a)v}{1+} \frac{v}{1+} \frac{(2-a)v}{1+} \frac{2v}{1+} \cdots \]
\item[\bf(D)] For $n\geq0$; (though claimed to be of limited use unless $a\sim-n$)
\[ \Gam(a+n+1, z) = \Gam(a+n, z) + \frac{\Gam(a)\exp(-z) z^{n+a}}{\Gam(a+n+1)} \]
\end{itemize}
\end{implementation}
...
\item *OR* use
    ...
    and note that the $J_{n+1/2}$ are easy to evaluate
\end{itemize}
\end{implementation}
